"""
SGD baseline for spatial GMM clustering.
"""
import torch
import time
import os
...
def run(cfg: dict):
    ...
    output_folder = os.path.join(cfg['data']['output_dir'], f"sgd_baseline_{max_iter}_seed_{seed}")
    ...
    for it in range(max_iter + 1):
        ...
        with torch.no_grad():
            alpha_all[St] = alpha_b - lr_alpha * alpha_b.grad
            m0 = m0_tmp - lr_m0 * m0_tmp.grad
            rho0 = rho0_tmp - lr_rho0 * rho0_tmp.grad
    ...
    results = {
        "_save_name": f"results_sgd_{max_iter}_seed_{seed}_lrA_{lr_alpha:.0e}_lrM_{lr_m0:.0e}_lrR_{lr_rho0:.0e}.pt",
        "algorithm": "sgd",
    ...
    save_and_plot(results, coords, final_z_pred, K, output_folder,
                  "final_labels.png", "Predicted labels (SGD baseline)")
