"""
RMSProp baseline for spatial GMM clustering.
"""
import torch
...
def run(cfg: dict):
    t = cfg['training']
    alg_cfg = cfg['algorithms']['rmsprop']
    ...
    rms_alpha = alg_cfg['alpha']
    rms_eps = alg_cfg['eps']
    rms_momentum = alg_cfg['momentum']
    rms_centered = alg_cfg['centered']
    ...
    output_folder = os.path.join(cfg['data']['output_dir'], f"rmsprop_baseline_{max_iter}_seed_{seed}")
    ...
    optimizer = torch.optim.RMSprop(
        [
            {"params": [alpha_all], "lr": lr_alpha},
            {"params": [m0], "lr": lr_m0},
            {"params": [rho0], "lr": lr_rho0},
        ],
        alpha=rms_alpha,
        eps=rms_eps,
    ...
    results = {
        "_save_name": f"results_rmsprop_{max_iter}_seed_{seed}_lrA_{lr_alpha:.0e}_lrM_{lr_m0:.0e}_lrR_{lr_rho0:.0e}.pt",
        "algorithm": "rmsprop",
        ...
        "rms_alpha": float(rms_alpha), "rms_eps": float(rms_eps),
        "rms_momentum": float(rms_momentum), "rms_centered": bool(rms_centered),
    ...
    save_and_plot(results, coords, final_z_pred, K, output_folder,
                  "final_labels.png", "Predicted labels (RMSProp baseline)")
