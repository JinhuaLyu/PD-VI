"""
Stochastic CAVI (SVI) baseline with Robbins-Monro (decaying) step size.
"""
import torch
import time
import os
...
def run(cfg: dict):
    t = cfg['training']
    alg_cfg = cfg['algorithms']['svi_faster']
    ...
    output_folder = os.path.join(cfg['data']['output_dir'], f"svi_faster_{max_iter}_seed_{seed}")
    ...
    results = {
        "_save_name": f"results_svi_faster_{max_iter}_seed_{seed}_tau_{tau:.0e}_sc_{scale:.2f}.pt",
        "algorithm": "svi_faster",
        ...
        "tau": float(tau), "scale": float(scale), "fp_steps": int(fp_steps),
    ...
    save_and_plot(results, coords, final_z_pred, K, output_folder,
                  "final_labels.png", "Predicted labels (SVI Faster / Robbins-Monro)")
