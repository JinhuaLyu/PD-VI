...
DATA_FILE     = f"{ROOT}/data/tep_embeddings.npz"
OUTPUT_DIR    = f"{ROOT}/output"

SEEDS        = [42, 1, 2, 3, 4]
SAMPLE_SIZE  = 500
MAX_ITER     = int(os.environ.get("MAX_ITER", "20000"))   # TEP: ~116 epochs over 172 time batches
T            = 1.0
LOG_EVERY    = 20
RESTART_EVERY = 100
...
_PAVI_H_CACHE = {}
_INIT_CACHE = {}      # cache kpp init per seed (KMeans on 86k x 64 is costly; deterministic)
...
# ----------------------------------------------------------------------------
# data: TEP SensorSCAN 64-d embeddings, kept in (run, time) order so consecutive
# chunks form time-contiguous (near single-class) batches.
# ----------------------------------------------------------------------------
_npz    = np.load(DATA_FILE)
_emb    = _npz["emb"].astype(np.float64)
_emb    = (_emb - _emb.mean(0)) / (_emb.std(0) + 1e-8)          # z-score per dim
# truncate to a multiple of the batch size so time-contiguous batches divide evenly
_keep   = (_emb.shape[0] // SAMPLE_SIZE) * SAMPLE_SIZE
_emb    = _emb[:_keep]
Y       = torch.from_numpy(_emb).to(DEVICE)
Z_TRUE  = torch.from_numpy(_npz["y"][:_keep].astype(np.int64)).to(DEVICE)
N, D    = Y.shape
K       = int(Z_TRUE.max().item()) + 1

# real data: no synthetic ground-truth Gaussians, so use empirical per-class mean
# and diagonal covariance for the Hungarian alignment in _finalize.
_zt = Z_TRUE.cpu().numpy()
TRUE_MEANS = np.stack([_emb[_zt == k].mean(0) for k in range(K)])                # (K, d)
TRUE_COVS  = np.stack([np.diag(_emb[_zt == k].var(0) + 1e-6) for k in range(K)])  # (K, d, d)
TRUE_WEIGHTS = np.array([float(np.mean(_zt == k)) for k in range(K)])
...
def make_time_batches(n, batch_size, device):
    """Time-contiguous batches: the data is already in (run, time) order, so each
    batch is a consecutive block of batch_size points (one batch ~ one fault class
    over a short time span -> non-IID / federated). Used for the TEP experiment."""
    nb = n // batch_size
    idx = torch.arange(nb * batch_size, device=device).view(nb, batch_size)
    return [idx[b].clone() for b in range(nb)]
...
def build_init(seed, init_mode="kpp", sigma_source="kpp"):
    ...
    """
    ck = (seed, init_mode, sigma_source)
    if init_mode == "kpp" and ck in _INIT_CACHE:   # deterministic; reuse the costly KMeans
        return _INIT_CACHE[ck]
    km = KMeans(n_clusters=K, init="k-means++", n_init=5, random_state=seed).fit(Y.cpu().numpy())
...
def _plugin_assign(means, sigma0_diag):
    ...
    = argmin_k sum_d (x_sd - m_kd)^2 / sigma0_d^2. Uses ONLY the means, so no
    method's own q(z) (PAVI's synth-alpha vs the others' iterated alpha) and no
    q(mu) variance enters; every method is scored by the same rule. Uses the
    matmul identity argmin||x-m||^2 = argmax(x.m - 0.5||m||^2) to avoid a big
    (N, K, D) broadcast on the 86k x 64 data."""
    inv = (1.0 / (sigma0_diag ** 2)).view(1, D)
    Yw = Y * inv                                       # (N, D)
    msq = 0.5 * (means * means * inv).sum(dim=1)       # (K,)
    score = Yw @ means.t() - msq.view(1, K)            # (N, K)
...
def _finalize(alpha_all, m0_final, sigma0_diag, sigma1_diag):
    ...
    # own q(z) (alpha_all is kept only for the call signature).
...
def run_method(method, seed, *, eta=None, lr=None, weight_decay=0.01,
    ...
    batches     = make_time_batches(N, batch_size=sample_size, device=DEVICE)  # TEP: time-contiguous
    ...
    # ----------------------------------------------- AdamW / SGD / RMSProp
    elif method in ("AdamW", "Adam", "SGD", "RMSProp"):
        ...
        elif method == "Adam":          # plain Adam, no (decoupled) weight decay
            optimizer = torch.optim.Adam(pg, betas=(0.9, 0.999), eps=1e-8)
        elif method == "SGD":           # plain SGD; momentum tunable (default 0.0 = bio sgd.py)
            optimizer = torch.optim.SGD(pg, momentum=momentum)
    ...
    # ----------------------------------- PAVI (particle mean-field VI baseline)
        ...
        for it in range(T_pavi + 1):
            ...
            if data_mode == "mb":
                Xb = Y[torch.randint(0, N, (M,), generator=gen, device=DEVICE)]
            else:
                Xb = Y
