...
For a trajectory we match independently at every logged step (the estimated
covariance is constant, so each step's Hungarian match is well-defined). The
final-step value equals the older fix-from-final-means convention, and per-step
matching additionally makes the shared initialization coincide across methods.
"""
import numpy as np
...
def w2_trajectory(m_traj, sigma0, true_means, true_covs):
    ...
    Returns (w2_mean (L,), perm) with per-step optimal matching (perm is the
    last step's). Because every method shares the same initialization, per-step
    matching makes iteration 0 coincide across methods; the final-step value is
    identical to fixing the permutation from the final means.
    """
    ...
    perm = None
    for t in range(L):
        perm, w2_per_true = match_components(m_traj[t], sigma0, true_means, true_covs)
        w2_mean[t] = float(w2_per_true.mean())
    return w2_mean, perm
